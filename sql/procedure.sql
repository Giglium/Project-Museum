...
DELIMITER $
CREATE PROCEDURE AumentoStipendio(IN CodiceDirettore smallint UNSIGNED, CodiceDipendente smallint UNSIGNED,
                                  percentuale int UNSIGNED)
BEGIN
    ...
    FROM Dipendente
    WHERE CodMuseo =
          (SELECT CodMuseo FROM Dipendente Dir WHERE Dir.CodiceDip = CodiceDirettore AND Dir.Lavoro = 'direttore')
      AND CodiceDip = CodiceDipendente;
    IF (Temp = CodiceDipendente) THEN
        UPDATE Dipendente
        SET Stipendio=Stipendio + ((Stipendio * percentuale) / 100)
        WHERE CodiceDip = Temp
          AND Lavoro <> 'direttore';
    END IF;
END$
