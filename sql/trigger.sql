...
BEGIN
    ...
    IF NEW.PartitaIVA IS NOT NULL THEN
        IF NEW.Ente = 2 THEN
            ...
            INSERT INTO Errori(Descrizione)
            VALUES (CONCAT('Il Fornitore Codice Fiscale: ', NEW.CodiceFiscale,
                           ' ha generato un errore nella tabella Fornitori. Un privato non ha partita IVA ! '));
        ELSE
            SET NEW.PartitaIVA = UPPER(NEW.PartitaIVA);
        END IF;
    END IF;
END$
...
BEGIN
    IF CURDATE() > NEW.InizioRestauro THEN
        ...
        INSERT INTO Errori(Descrizione)
        VALUES (CONCAT('Il reperto ID: ', NEW.CodA,
                       ' ha generato un errore nella tabella RepertoArtistico. InizioRestauro antecedente a oggi'));
    ELSE
        IF NEW.FineRestauro < NEW.InizioRestauro THEN
            ...
            INSERT INTO Errori(Descrizione)
            VALUES (CONCAT('Il reperto ID: ', NEW.CodA,
                           ' ha generato un errore nella tabella RepertoArtistico. InizioRestauro > FineRestauro !'));
        END IF;
    END IF;
END$
...
BEGIN
    IF NEW.FineRestauro < NEW.InizioRestauro THEN
        ...
        SET NEW.FineRestauro = NULL;
        INSERT INTO Errori(Descrizione)
        VALUES (CONCAT('Il reperto ID: ', NEW.CodS,
                       ' ha generato un errore nella tabella RepertoStorico. InizioRestauro > FineRestauro ! '));
    END IF;
END$
...
BEGIN
    IF NEW.DataMorte < NEW.DataNascita THEN
        ...
        INSERT INTO Errori(Descrizione)
        VALUES (CONCAT('Errore generato da ID: ', NEW.Nome, ' ', New.Cognome,
                       ' nella tabella Artista. DataMorte > DataNascita ! '));
    END IF;
END$
